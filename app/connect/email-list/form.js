  ...
  const handleSubmit = (data) => {
    ...
    startTransition(() => {
      ...
      postContact({ name, email }).then((json) => {
        ...
        if (json.Count > 0) {
          message = "Success! Keep an eye out on your inbox for updates";
          ...
          error =
            "Error. Try again or email mikek222@gmail.com to be added manually";
        }
  ...
  return (
    <form action={handleSubmit} className="indent grid gap-[2px] w-[270px]">
      ...
      <button
        type="submit"
        disabled={isPending}
        className={`bg-blue-700 text-center text-white w-[130px] h-[36px] mt-[4px] disabled:bg-slate-50 disabled:text-slate-500`}
      >
        {isPending ? 
